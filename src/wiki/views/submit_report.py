...
from ..models.account import Report, InfractionEvent, Privilege, User, DiscussionBoard, DiscussionReport
...
class ApproveReportView(View):
    def post(self, request, *args, **kwargs):
        ...
        if privilege == 'discussion':
            ...
            if approval == 'Infraction' and DiscussionReport.objects.filter(post_id = post_id).exists():
                post = DiscussionBoard.objects.get(post_id = post_id)
                privilege = Privilege.objects.get_or_create(name="Commenting", user=post.user)
